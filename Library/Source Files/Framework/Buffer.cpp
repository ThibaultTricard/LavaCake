...
namespace LavaCake {
  namespace Framework {
    ...
    void Buffer::copyToImage(CommandBuffer& cmdBuff, Image& image, const VkBufferImageCopy& region) {
      vkCmdCopyBufferToImage(cmdBuff.getHandle(), m_buffer, image.getHandle(), image.getLayout(), 1u, &region);
    }
    ...
    void Buffer::copyToBuffer(CommandBuffer& cmdBuff, Buffer& buffer, const VkBufferCopy& region) {
      vkCmdCopyBuffer(cmdBuff.getHandle(), m_buffer, buffer.m_buffer, 1u, &region);
    }
