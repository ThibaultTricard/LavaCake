	...
	namespace Framework {
		...
		void PushConstant::addArray(const std::string& name, std::vector<int>& value) {
			if (m_variableNames.find(name) != m_variableNames.end()) {
				ErrorCheck::setError("Variable name allready taken in this PushConstant",1);
				return;
			}
		...
		void PushConstant::setArray(const std::string& name, std::vector<int>& value) {
			if (m_variableNames.find(name) == m_variableNames.end()) {
				ErrorCheck::setError("Variable name does not exist in this PushConstant",1);
				return;
			}
			int i = m_variableNames[name];
			std::vector<int> v = m_variables[i];
			if (v.size() != value.size()) {
				ErrorCheck::setError("The new value does not match the type of the one currently stored in this PushConstant",1);
				return;
			}
