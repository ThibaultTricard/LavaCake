			...
			bool CreateLogicalDeviceWithGeometryShadersAndGraphicsAndComputeQueues(VkInstance   instance,
				...
				for (auto & physical_device : physical_devices) {
					...
					if (!CreateLogicalDevice(physical_device, requested_queues, {}, &device_features, logical_device)) {
						...
						Queue::GetDeviceQueue(logical_device, graphics_queue_family_index, 0, graphics_queue);
						Queue::GetDeviceQueue(logical_device, compute_queue_family_index, 0, compute_queue);
						return true;
					}
				}
				return false;
