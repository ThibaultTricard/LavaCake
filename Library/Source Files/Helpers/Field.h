...
namespace LavaCake {
  ...
  class Field2DGrid : public Field2D<T>{
    ...
    Field2DGrid(std::vector<T>& data, uint32_t width, uint32_t height, ABBox<2> boundingbox): Field2D<T>(){
      m_fields = data;
      m_width = width;
      m_height = height;
    ...
    T sample(vec2f pos) override{
      vec2f X = (pos - m_boundingbox.min()) / (m_boundingbox.diag());
      ...
      B = m_fields[U[0]+1 + (U[1]+1) *m_width];
      C = m_fields[U[0]   +  U[1]    *m_width];
  ...
  public:
    ...
    Field3DGrid(std::vector<T>& data, uint32_t width, uint32_t height, uint32_t depth, ABBox<3> boundingbox){
      m_fields = data;
      m_width = width;
      m_height = height;
      m_depth = depth;
    ...
    T sample(vec3f pos) override{
      ...
      B = m_fields[U[0]+1 + (U[1]+1) *m_width + U[2]    *m_width*m_height];
      C = m_fields[U[0]   +  U[1]    *m_width + U[2]    *m_width*m_height];
      D = m_fields[U[0]+1 + (U[1]+1) *m_width + U[2]    *m_width*m_height];
      E = m_fields[U[0]   +  U[1]    *m_width + (U[2]+1)*m_width*m_height];
      F = m_fields[U[0]+1 + (U[1]+1) *m_width + (U[2]+1)*m_width*m_height];
      G = m_fields[U[0]   +  U[1]    *m_width + (U[2]+1)*m_width*m_height];
